...
public final class GCMRegistrar
{
	 ...
	 * 
	 * <p>
	 * This instance cannot be the same as the one defined in the manifest because it needs a different permission.
	 */
	 ...
	 * <li>The {@link BroadcastReceiver}(s) uses the {@value com.google.android.gcm.GCMConstants#PERMISSION_GCM_INTENTS}
	 * permission.
	 ...
	 * <p>
	 * This method should be used during development time to verify that the manifest is properly set up, but it doesn't
	 * need to be called once the application is deployed to the users' devices.
	 * 
	 * @param context
	 *            application context.
	...
	public static void checkManifest(Context context)
	{
		...
		log(context, Log.VERBOSE, "number of receivers for %s: %d", packageName, receivers.length);
	...
	private static void checkReceiver(Context context, Set<String> allowedReceivers, String action)
	{
		...
		log(context, Log.VERBOSE, "Found %d receivers for action %s", receivers.size(), action);
	 ...
	 * <p>
	 * The result will be returned as an {@link GCMConstants#INTENT_FROM_GCM_REGISTRATION_CALLBACK} intent with either a
	 * {@link GCMConstants#EXTRA_REGISTRATION_ID} or {@link GCMConstants#EXTRA_ERROR}.
	 * 
	 * @param context
	 *            application context.
	 * @param senderIds
	 *            Google Project ID of the accounts authorized to send messages to this application.
	 * @throws IllegalStateException
	 ...
	 * <p>
	 * The result will be returned as an {@link GCMConstants#INTENT_FROM_GCM_REGISTRATION_CALLBACK} intent with an
	 ...
	 * 
	 * <p>
	 * This method should be called by the main activity's {@code onDestroy()} method.
	 */
	...
	private synchronized static void setPackageNameExtra(Context context, Intent intent)
	{
		if (sAppPendingIntent == null)
		{
			log(context, Log.VERBOSE, "Creating pending intent to get package name");
			sAppPendingIntent = PendingIntent.getBroadcast(context, 0, new Intent(), 0);
		}
		intent.putExtra(GCMConstants.EXTRA_APPLICATION_PENDING_INTENT, sAppPendingIntent);
	}
	...
	static synchronized void setRetryBroadcastReceiver(Context context)
	{
		if (sRetryReceiver == null)
		{
			...
			{
				// should never happen
				log(context, Log.ERROR, "internal error: retry receiver class not set yet");
			...
			else
			{
				...
				{
					log(context, Log.ERROR, "Could not create instance of %s. " + "Using %s directly.", sRetryReceiverClassName, GCMBroadcastReceiver.class.getName());
					sRetryReceiver = new GCMBroadcastReceiver();
				}
			}
	 ...
	 */
	static synchronized void setRetryReceiverClassName(Context context, String className)
	{
		log(context, Log.VERBOSE, "Setting the name of retry receiver class to %s", className);
	...
	public static String getRegistrationId(Context context)
	{
		...
		{
			log(context, Log.VERBOSE, "App version changed from %d to %d;" + "resetting registration id", oldVersion, newVersion);
	...
	private static void setRegisteredOnServer(Context context, Boolean flag, long expirationTime)
	{
		...
		if (flag != null)
		{
			...
			log(context, Log.VERBOSE, "Setting registeredOnServer flag as %b until %s", flag, new Timestamp(expirationTime));
		}
		else
		{
			log(context, Log.VERBOSE, "Setting registeredOnServer expiration to %s", new Timestamp(expirationTime));
		}
	...
	public static boolean isRegisteredOnServer(Context context)
	{
		...
		if (isRegistered)
		{
			...
			{
				log(context, Log.VERBOSE, "flag expired on: %s", new Timestamp(expirationTime));
				return false;
			}
		}
		return isRegistered;
	 ...
	 * <p>
	 * This method should be called after a GCM call fails, passing an exponential value.
	 * 
	 * @param context
	 *            application's context.
	 * @param backoff
	 ...
	 *            list of arguments
	 */
	private static void log(Context context, int priority, String template, Object... args)
	{
		if (Log.isLoggable(TAG, priority))
		{
			String message = String.format(template, args);
			Log.println(priority, TAG, "[" + context.getPackageName() + "]: " + message);
		}
	}
